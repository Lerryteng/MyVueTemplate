<template>
  <div class="loadmore">
    <div class="content" :class="{ 'is-dropped': topDropped || bottomDropped}"
         :style="{ 'transform': 'translate3d(0, ' + translate + 'px, 0)' ,'transition-duration':animateTime + 's'}">
      <slot name="top">
      ...
      <slot name="bottom" v-if="bottomType === 'drop' && canInif">
        <div class="bottom" v-if="bottomMethod">
          ...
          <span class="text">{{ newBottomText }}</span>
        </div>
      </slot>
      <slot name="bottom" v-if="canInif && bottomType != 'drop'">
        <div v-if="bottomMethod" class="inifScroll" :style="{'transition-duration':btAnimateTime + 's'}">
          <spinner width="30" type="triple-bounce" color="#f3674f"></spinner>
          <span class="text">{{ newBottomText }}</span>
        </div>
      </slot>
    </div>
...
<script type="text/babel">
  ...
  export default {
    ...
    // topParam="0", bottomParam="1", 在调用组件的时候传入，在执行下拉刷新和滚动加载的时候返回
    props: {
      maxDistance: {
        type: Number,
        default: 0
    ...
    watch: {
      ...
      bottomStatus(val) {
        ...
        switch (val) {
          case 'pull':
            this.newBottomText = this.bottomType === 'drop' ? this.bottomPullText : this.bottomInfiniteEndText;
            break;
          case 'drop':
            this.newBottomText = this.bottomDropText;
            break;
          case 'loading':
            this.newBottomText = this.bottomType === 'drop' ? this.bottomLoadingText : this.bottomInfiniteLoadingText;
            break;
        }
      }
    },
...
//          }, 200);
          // sticky会脱离文档流，导致loadMore出现问题，这里先通过节流解决sticky导致的无线加载多次请求数据问题
